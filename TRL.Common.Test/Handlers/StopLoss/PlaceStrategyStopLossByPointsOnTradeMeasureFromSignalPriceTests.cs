...
{
    [TestClass]
    public class PlaceStrategyStopLossByPointsOnTradeMeasureFromSignalPriceTests
    {
        ...
        public void Setup()
        {
            ...
            this.handler =
                new PlaceStrategyStopLossByPointsOnTrade(this.strategyHeader, this.tradingData, this.signalQueue, new NullLogger(), true);
        ...
        public void make_signal_to_sell_on_stop_for_long_position_with_single_open_trade_test()
        {
            ...
            Assert.AreEqual(openSignal.Price - this.spSettings.Points, closeSignal.Stop);
        ...
        {
            Signal openSignal = new Signal(this.strategyHeader, DateTime.Now, TradeAction.Buy, OrderType.Market, 125000, 0, 0);
            Trade firstTrade = this.tradingData.AddSignalAndItsOrderAndTrade(openSignal, openSignal.Price - 10, 5);
            Assert.AreEqual(0, this.signalQueue.Count);
        ...
        {
            Signal openSignal = new Signal(this.strategyHeader, DateTime.Now, TradeAction.Buy, OrderType.Market, 125000, 0, 0);
            Trade firstTrade = this.tradingData.AddSignalAndItsOrderAndTrade(openSignal, openSignal.Price + 20, 5);
            Assert.AreEqual(0, this.signalQueue.Count);

            firstTrade.Order.Cancel(DateTime.Now, "cancel order");
            Assert.IsTrue(firstTrade.Order.IsCanceled);

            Trade secondTrade = this.tradingData.AddSignalAndItsOrderAndTrade(openSignal, openSignal.Price + 20, 3);
        ...
        public void make_signal_to_buy_on_stop_for_short_position_with_single_open_trade_test()
        {
            ...
            Trade trade = this.tradingData.AddSignalAndItsOrderAndTrade(openSignal, openSignal.Price + 20);
