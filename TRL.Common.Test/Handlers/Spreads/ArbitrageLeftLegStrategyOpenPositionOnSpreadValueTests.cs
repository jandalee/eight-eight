...
{
    [TestClass]
    public class ArbitrageLeftLegStrategyOpenPositionOnSpreadValueTests
    {
        ...
        [TestMethod]
        public void Make_Signal_To_Sell_Spread_LeftLeg_test()
        {
            ...
            Assert.AreEqual(this.lStrategyVolumeChangeStep.Amount, lastSignal.Amount);
        }

        [TestMethod]
        public void Make_Signal_To_Buy_Spread_LeftLeg_test()
        {
            Assert.AreEqual(0, this.signalQueue.Count);
        ...
        {
            Signal openSignal = new Signal(this.lStrategy, DateTime.Now, TradeAction.Sell, OrderType.Market, 1, 0, 0);
        ...
        {
            Signal openSignal = new Signal(this.lStrategy, DateTime.Now, TradeAction.Buy, OrderType.Market, 1, 0, 0);
            this.tradingData.Get<ICollection<Signal>>().Add(openSignal);
        ...
        public void Change_signal_to_buy_amount_if_volume_step_greater_than_unfilled_amount_test()
        {
            ...
            Assert.AreEqual(-9, this.tradingData.GetAmount(this.lStrategy));
            Assert.AreEqual(1, this.tradingData.Get<IEnumerable<Signal>>().Count());
