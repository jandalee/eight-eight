...
{
    [TestClass]
    public class ExitSpreadOnQuoteChangeTests : TraderBaseInitializer
    {
        ...
        public void Setup()
        {
            ...
            ExitSpreadOnQuoteChange handler =
                new ExitSpreadOnQuoteChange(this.qProvider,
                    this.leftLeg,
                    this.rightLeg,
                    this.spreadSettings,
                    this.tradingData,
        ...
        [TestMethod]
        public void ExitSpreadOnQuote_make_signals_to_close_existing_position()
        {
            ...
            this.qProvider.Update(0, "RTS-12.13_FT", 145000, 20, 145010 , 100);
            this.qProvider.Update(0, "Si-12.13_FT", 31999, 50, 32000, 40);
            this.qProvider.Update(0, "Eu-12.13_FT", 43999, 30, 43000, 0);
            ...
            Assert.AreEqual(TradeAction.Sell, this.tradingData.Make<Order>().ElementAt(3).TradeAction);
            Assert.AreEqual(TradeAction.Buy, this.tradingData.Make<Order>().ElementAt(4).TradeAction);
            Assert.AreEqual(TradeAction.Buy, this.tradingData.Make<Order>().ElementAt(5).TradeAction);
        ...
        [TestMethod]
        public void ExitSpreadOnQuote_ignore_updates_when_no_some_of_symbol_quotes_in_storage()
        {
        ...
        [TestMethod]
        public void ExitSpreadOnQuote_ignore_updates_when_no_position_exists()
        {
            Assert.AreEqual(0, this.tradingData.Get<IEnumerable<Signal>>().Count());
